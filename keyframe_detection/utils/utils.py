...
def save_checkpoint(state, is_best=False, gap=1, filename='models/checkpoint.pth.tar', 
                   keep_all=False, max_best_models=5):
    """
    Save model checkpoint and manage historical checkpoints.
    
    Args:
        state: Model state dict and other training info
        is_best: Whether this is the best model so far
        gap: Gap between epochs to keep (for historical cleanup)
        filename: Path to save current checkpoint
        keep_all: Whether to keep all historical checkpoints
        max_best_models: Maximum number of best models to keep
    """
    # Save current checkpoint
    torch.save(state, filename)
    
    # Clean up historical checkpoint files
    if not keep_all and gap > 0:
        last_epoch_path = os.path.join(
            os.path.dirname(filename),
            f'epoch{state["epoch"] - gap}.pth.tar'
        )
        try:
            os.remove(last_epoch_path)
        except FileNotFoundError:
            pass  # File not existing is normal
        except Exception as e:
            print(f"Warning: Failed to remove {last_epoch_path}: {e}")
    
    # Save best model and manage historical best models
    if is_best:
        best_filename = os.path.join(
            os.path.dirname(filename),
            f'model_best_epoch{state["epoch"]}.pth.tar'
        )
        torch.save(state, best_filename)
        
        # Manage best models count
        try:
            # Get all best model files
            past_best = glob.glob(
                os.path.join(os.path.dirname(filename), 'model_best_epoch*.pth.tar')
            )
            ...
            past_best = sorted(past_best, key=extract_epoch_number)
            
            # If exceed limit, remove oldest ones
            while len(past_best) > max_best_models:
                oldest_model = past_best.pop(0)
                try:
                    os.remove(oldest_model)
                except Exception as e:
                    print(f"Warning: Failed to remove {oldest_model}: {e}")
                    
        except Exception as e:
            print(f"Warning: Failed to manage best models: {e}")
