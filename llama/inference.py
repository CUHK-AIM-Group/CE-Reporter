...
def setup_model(mode_name, gpu_id):
    """Setup merged model and tokenizer (no LoRA)"""
    # Set GPU device
    os.environ["CUDA_VISIBLE_DEVICES"] = str(gpu_id)
    torch.cuda.empty_cache()

    # Load merged model directly
    tokenizer = AutoTokenizer.from_pretrained(mode_name, use_fast=True, device_map={"": 0}, trust_remote_code=True)
    model = AutoModelForCausalLM.from_pretrained(mode_name, device_map={"": 0}, trust_remote_code=True, rope_scaling=None)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Generate responses using merged LLM')
    parser.add_argument('--mode_name', type=str,  
                       default="CE-Reporter-llama-3.1-8b",  
    ...
    model, tokenizer = setup_model(args.mode_name, args.gpu_id)
